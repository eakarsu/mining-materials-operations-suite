const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:4670';
...
const adminCookie = await login('admin@mining-materials.local', 'admin123');
const managerCookie = await login('manager@mining-materials.local', 'manager123');
const analystCookie = await login('analyst@mining-materials.local', 'analyst123');

await expectJson('/api/dashboard', adminCookie);
await expectJson('/api/entities/sites', analystCookie);
await expectJson('/api/documents', analystCookie);
await expectStatus('/api/documents/upload', analystCookie, 405);

const records = await expectJson('/api/entities/sites', managerCookie);
const rowId = records.rows[0].id;

const approveResponse = await fetch(`${baseUrl}/api/entities/sites/approve`, {
  method: 'POST',
  headers: {
...
const forbiddenApprove = await fetch(`${baseUrl}/api/entities/sites/approve`, {
  method: 'POST',
  headers: {
